...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	//++ Диплом
	// регистр ВКМ_ДанныеИзРеализаций
	Движения.ВКМ_ДанныеИзРеализаций.Записывать = Истина;
	ДвижениеДанные = Движения.ВКМ_ДанныеИзРеализаций.Добавить();
	ДвижениеДанные.Период = Дата;
	ДвижениеДанные.Контрагент = Контрагент;
	ДвижениеДанные.Договор = Договор;
	//ДвижениеДанные.Номенклатура = Номенклатура;
	...
	Для Каждого ТекСтрокаТовары Из Товары Цикл
		...
		//++ Диплом
		ДвижениеДанные.Номенклатура = ТекСтрокаТовары.Номенклатура;
		ДвижениеДанные.Сумма = ТекСтрокаТовары.Сумма;
		//конец
	КонецЦикла;
...
Процедура ВыполнитьАвтозаполнение() Экспорт
	...
	Запрос.Текст = 
		...
		|	ЕСТЬNULL(ВКМ_ВыполненныеКлиентуРаботыОбороты.Договор.ВКМ_СтоимостьЧасаРабот, 0) КАК СтоимостьЧасаРабот,
		|	ЕСТЬNULL(ВКМ_ВыполненныеКлиентуРаботыОбороты.Договор.ВКМ_СуммаАбонентскойПлатыЕжемесячно, 0) КАК СуммаАбПлаты
		|ИЗ
		...
		//"ВЫБРАТЬ
		//|	ЕСТЬNULL(РеализацияТоваровУслуг.Договор.ВКМ_СуммаАбонентскойПлатыЕжемесячно, 0) КАК СуммаАбПлаты,
		//|	ЕСТЬNULL(ВКМ_ВыполненныеКлиентуРаботыОбороты.КоличествоЧасовОборот, 0) КАК КоличествоЧасов,
		//|	ЕСТЬNULL(ВКМ_ВыполненныеКлиентуРаботыОбороты.СуммаКОплатеОборот, 0) КАК СуммаКОплате,
